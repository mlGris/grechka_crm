...
async def register_user(user_data: SUserRegister, session: AsyncSession = TransactionSessionDep) -> dict:
    user = await UserDAO.find_one_or_none(session=session, filters=EmailModel(email=user_data.email))
    if user:
        raise UserAlreadyExistsException
    user_data_dict = user_data.model_dump()
    del user_data_dict['confirm_password']
    await UserDAO.add(session=session, values=SUserAddDB(**user_data_dict))
...
async def get_all_users(session: AsyncSession = SessionDep,
    ...
    return await UserDAO.find_all(session=session, filters=None)
